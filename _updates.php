...
<div class="meldung_erfolg">
	...
	<ul>
		...
		<li>Automatisierter, regelm‰ﬂiger Newsletter-Versand per <a href="https://www.01-scripts.de/01newsletter.php?install#paternoster" target="_blank">Paternoster-Mailing-Funktion</a></li>
...
elseif(isset($_REQUEST['update']) && $_REQUEST['update'] == "120_zu_130"){
	...
	// Neue Datenbanktabelle f¸r Versand per _cronjob.php
...
elseif(isset($_REQUEST['update']) && $_REQUEST['update'] == "1001_zu_1100"){
	...
	// Neue Einstellungen anlegen:
	$sql_insert = "INSERT INTO ".$mysql_tables['settings']." (modul,is_cat,catid,sortid,idname,name,exp,formename,formwerte,input_exp,standardwert,wert,nodelete,hide) VALUES
				('".$mysqli->escape_string($modul)."','0','1','5','send_benachrichtigung','E-Mail-Benachrichtigung','M&ouml;chten Sie eine Benachrichtigung per E-Mail erhalten, wenn sich ein neuer Benutzer f¸r den Newsletter anmeldet?','Aktivieren|Deaktivieren','1|0','','0','0','0','0'),
	...
	// Neue Spalte attachments an Archiv-Tabelle anf¸gen
...
elseif(isset($_REQUEST['update']) && $_REQUEST['update'] == "1000_zu_1001"){
	...
	$sql_insert = "INSERT INTO ".$mysql_tables['settings']." (modul,is_cat,catid,sortid,idname,name,exp,formename,formwerte,input_exp,standardwert,wert,nodelete,hide) VALUES
				...
				('".$mysqli->escape_string($modul)."','0','1','4','versand_altname','Alternativtext f¸r Versandadresse','','text','50','','','','0','0');";
	$result = $mysqli->query($sql_insert) OR die($mysqli->error);
